...
const String FIRMWARE_VER = "1.0.0";               // versão deste firmware (tags do repo: v1.0.0, v1.0.1, ...)
...
String campeaoAtual = "";
String avisoAtual = "";
const int VERSAO_PAGINA = 18;   // incrementar a cada mudanca no JS servido (placar_html.h)
...
void setup() {
  ...
  server.on("/generate_204", servirHTML);
  server.on("/hotspot-detect.html", servirHTML);
  server.on("/success.txt", servirHTML);
  server.on("/ncsi.txt", servirHTML);
  server.on("/connecttest.txt", servirHTML);
